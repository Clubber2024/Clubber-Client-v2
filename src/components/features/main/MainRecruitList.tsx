...
const fetchData = async()=>{
  ...
  return (
    <>
      <h2 className="font-bold text-lg py-3 mt-2 cursor-pointer" onClick={()=>router.push('/recruitList')}>모집글</h2>
      <Card className="p-6 gap-2">
        {recruitData?.map((recruit, idx) => (
          <div key={idx} className="p-0 flex flex-row items-center justify-between text-sm cursor-pointer hover:text-primary" onClick={()=>router.push(`/recruitList/${recruit.recruitId}`)}>
            <div>
              <Badge
