...
const PercentageBar = ({ text, count, total }: { text: string, count: number, total: number }) => {
    ...
    return (
            <div className="h-[50px] w-full bg-[#f2f2f2] rounded-[10px] overflow-hidden relative">
                ...
                >
                    <span className="text-black bg-transparent font-bold whitespace-nowrap overflow-visible pl-[7px] md:text-[15px] text-[12.5px] md:font-[700] font-[600]">{text}</span>
  ...
  return (
    ...
    <div className="flex flex-col gap-2 mx-auto w-[60%]">
      ...
      <div><ReviewStats data={sortedReviews} /></div>
    </div>
  );
}
