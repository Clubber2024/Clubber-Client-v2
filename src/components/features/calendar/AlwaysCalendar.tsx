...
export default function AlwaysCalendar({ alwaysCalendars }: AlwaysCalendarProps) {
  return (
    <div className="h-full bg-primary p-2 md:p-4 rounded-xl md:rounded-xl md:my-10 mt-2">
      <Card className="bg-white p-6 md:rounded-none h-full gap-2 md:gap-6">
        {/* 헤더 */}
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-md md:text-xl font-bold text-gray-900 mr-0.5">이달의 상시모집</h2>
        ...
        <div className="flex flex-row md:flex-col space-x-2 md:space-x-0 md:space-y-2">
          {alwaysCalendars.length > 0 ? (
            alwaysCalendars.map((calendar) => (
              <Button
                key={calendar.clubId}
                className="text-xs md:text-md w-fit px-2 md:px-3 md:py-0.5 justify-start rounded-md transition-colors bg-secondary text-gray-800 hover:bg-primary/70"
              >
                {calendar.clubName}
              </Button>
            ))
